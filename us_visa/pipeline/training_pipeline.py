...
class TrainingPipeline:
    ...
    def start_model_pusher(
        ...
        """
        Execute the Model Pusher component.

        The Model Pusher uploads the evaluated trained model to the
        configured AWS S3 model registry.
        ...
        Args:
            model_evaluation_artifact:
                Artifact generated by the Model Evaluation stage.
                Contains the path of the trained model and the model
                evaluation result.

        Returns:
            ModelPusherArtifact:
                Contains the AWS S3 bucket name and the S3 model path.

        Raises:
            USvisaException:
                If an error occurs during the Model Pusher stage.
        """

        try:

            # ----------------------------------------------------------------------
            # Step 1: Log the beginning of Model Pusher stage.
            # ----------------------------------------------------------------------

            logging.info(
                ">>> STAGE 6: MODEL PUSHER — Starting..."
            )


            # ----------------------------------------------------------------------
            # Step 2: Initialize the Model Pusher component.
            #
            # Pass the Model Evaluation artifact so that the component can
            # access the evaluated trained model.
            # ----------------------------------------------------------------------
            ...
            # ----------------------------------------------------------------------
            # Step 3: Execute Model Pusher.
            #
            # The trained model is uploaded to the configured AWS S3 bucket.
            # ----------------------------------------------------------------------
            ...
            logging.info(
                ">>> STAGE 6: MODEL PUSHER — Completed."
            )
    ...
    def run_pipeline(self) -> None:
        ...
        try:
            ...
            # Check whether the newly trained model passed Model Evaluation.
            #
            # If the new model does not perform better than the existing
            # production model, do not push it to the S3 model registry.

            if not model_evaluation_artifact.is_model_accepted:

                logging.info(
                    "Model Evaluation Result: Model was NOT accepted."
                )

                logging.info(
                    "Skipping Model Pusher stage."
                )
            ...
            # --------------------------------------------------------------------------
            # Push the accepted model to AWS S3.
            # --------------------------------------------------------------------------

            logging.info(
                "Model Evaluation Result: Model was accepted."
            )
